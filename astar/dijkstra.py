...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            node.parentf = None
            node.queue_entryf = None
            node.parentb = None
            node.queue_entryb = None
            node.visitedf = False
            node.visitedb = False
        ...
        # Add NodeDistancePair to Q
        pqf = PriorityQueue()
        pqb = PriorityQueue()
        source.queue_entryf = NodeDistancePair(source, weight(source, destination))
        destination.queue_entryb = NodeDistancePair(destination, weight(source, destination))
        pqf.insert(source.queue_entryf)
        pqb.insert(destination.queue_entryb)
        
        
        while (len(pqf) > 0) and (len(pqb) > 0): 
            smallest_node_pair = pqf.extract_min()
            visitedf += 1
            node, dist = smallest_node_pair.node, smallest_node_pair.distance
            if node is destination:
                print("Error on djikstra optimized front")
                break
            for neighbor in node.adj:
                neighbor_dist = dist + weight(node, neighbor) + weight(neighbor, destination) - weight(node, destination)
                if neighbor.queue_entryf is None:
                    neighbor.parentf = node
                    neighbor.queue_entryf = NodeDistancePair(neighbor, neighbor_dist)
                    pqf.insert(neighbor.queue_entryf)
                elif neighbor_dist < neighbor.queue_entryf.distance:
                    neighbor.queue_entryf.distance = neighbor_dist
                    pqf.decrease_key(neighbor.queue_entryf)
                    neighbor.parentf = node
            node.visitedf = True
            if ((node.visitedb == True) and (node.visitedf == True)):
                #print(node)
                break
            
            smallest_node_pair = pqb.extract_min()
            visitedb += 1
            nodeb, dist = smallest_node_pair.node, smallest_node_pair.distance
            if nodeb is source:
                print("Error on djikstra optimized back")
                break
            for neighbor in nodeb.adj:
                neighbor_dist = dist + weight(nodeb, neighbor) + weight(neighbor, source) - weight(nodeb, source)
                if neighbor.queue_entryb is None:
                    neighbor.parentb = nodeb
                    neighbor.queue_entryb = NodeDistancePair(neighbor, neighbor_dist)
                    pqb.insert(neighbor.queue_entryb)
                elif neighbor_dist < neighbor.queue_entryb.distance:
                    neighbor.queue_entryb.distance = neighbor_dist
                    pqb.decrease_key(neighbor.queue_entryb)
                    neighbor.parentb = nodeb
            nodeb.visitedb = True
            
            if ((nodeb.visitedb == True) and (nodeb.visitedf == True)):
                #print(nodeb)
                break
                    
        #now search for minimum f and b distance sum
        min_distance = 100000000000
        min_node = None
        for node in nodes:
            if (node.queue_entryf is not None) and (node.queue_entryb is not None):
                min_d = node.queue_entryf.distance + node.queue_entryb.distance
                if min_d < min_distance:
                    min_distance = min_d
                    min_node = node
        #print(min_node)
        #from start to min_node and from destination to min_node
        path = []
        destinationf = min_node
        while destinationf is not None:
            path.append(destinationf)
            destinationf = destinationf.parentf
        path.reverse()
        
        pathb = []
        destinationb = min_node
        while destinationb is not None:
            pathb.append(destinationb)
            destinationb = destinationb.parentb
